...
nbici.factory('SessionService', ['$http', '$rootScope', 'localStorageService', function($http, $rootScope, localStorageService){
    ...
    var createSession = function (user) {
        Session = new User(user.id, user.first_name, user.last_name, user.email, user.classes_left, user.last_class_purchased, user.active, user.coupon, user.coupon_value, user.credits);
